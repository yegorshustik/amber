...
<template>
    <div class="wx-input-wrapper">
        <select v-if="name" :name="name" :multiple="multiple" :value="!multiple ? nativeValue : null" class="wx-native-hidden-select">
        ...
        <wx-dropdown ref="dropdownRef" :disabled="disabled" :closeOnClick="!props.multiple" class="wx-select">
            <template #trigger>
                <div
                    class="wx-select-trigger"
                    :class="{ disabled: disabled, invalid: getErrors().length > 0, filled: multiple ? currentValue?.length > 0 : currentValue }"
                >
                            ...
                            <template v-if="currentValue?.length > 0">
                                <div class="wx-select-tags">
                                    <span v-for="val in currentValue" :key="typeof val === 'object' ? val.value : val" class="wx-select-tag">
                                        ...
                                        <i
                                            class="remove"
                                            @click.stop="handleSelect(options.find((o) => o.value === (typeof val === 'object' ? val.value : val))!)"
                                            >×</i
                                        >
                                    </span>
                                </div>
