...
import {
    WxButton, WxButtons, WxCard,
    WxForm, WxFormControl, WxInput, WxPage,
    WxTextarea
} from '@/ui';
    ...
    <wx-page :heading="$t('inbox.forms.application.create')" :back="{ name : 'inbox.forms' }" v-if="loaded">
        <wx-form action="inbox/application" method="post" id="inbox-application-form" @success="() => success()">
            ...
            <wx-card class="inbox-application mx-auto" :title="useLocalesStore().selectLocalizedValue(form.title)">
                <div class="row">
                    <div class="col-12" :class="{'col-lg-6' : !((field as Record<string,any>).is_fullsize)}" v-for="(field) in (form as any).fields" :key="`form-item-${field.id}`">
                        <wx-form-control :title="useLocalesStore().selectLocalizedValue(field.title)">
                            ...
                            <template v-if="field.type.value === 'email'">
                                <wx-input type="email" :name="`field_${field.id}`" :placeholder="useLocalesStore().selectLocalizedValue(field.placeholder)" />
                            </template>
                            <template v-if="field.type.value === 'tel'">
                                <wx-input type="tel" :name="`field_${field.id}`" :placeholder="useLocalesStore().selectLocalizedValue(field.placeholder)" />
                            </template>
                ...
                <wx-buttons class="justify-content-end">
                    <wx-button theme="success" type="submit">{{ useLocalesStore().selectLocalizedValue(form.options['design.submit-button-text'], $t('send')) }}</wx-button>
                </wx-buttons>
            </wx-card>
        </wx-form>
    </wx-page>
